...
export function LanguageToggle({ className = "", compact = false }: Props) {
  ...
  return (
    <div className={`flex items-center rounded-full border border-ink-100/60 bg-cream-50/50 p-1 backdrop-blur-sm ${className}`.trim()} aria-label="Language switcher">
          ...
          return (
            <button
              ...
              className={`relative px-4 py-1.5 text-[9px] font-bold uppercase tracking-[0.15em] rounded-full transition-all duration-300 flex items-center gap-1.5 ${
                active ? "bg-ink-900 text-cream-50 shadow-sm" : "text-ink-500 hover:text-ink-900 hover:bg-cream-200/50"
              }`}
              aria-pressed={active}
            >
