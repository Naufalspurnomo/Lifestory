"use client";

import { useSession } from "next-auth/react";
import Link from "next/link";
import { useLanguage } from "../../components/providers/LanguageProvider";
import { Button } from "../../components/ui/Button";

const WHATSAPP_NUMBER = "6281234567890"; // Replace with actual number

export default function SubscribePage() {
  const { data: session } = useSession();
  const { locale } = useLanguage();
  const user = session?.user;
  const isSubscribed = user?.subscriptionActive;

  const copy =
    locale === "id"
      ? {
          plans: [
            {
              name: "Paket Keluarga",
              price: "Rp 99.000",
              period: "/ bulan",
              features: [
                "Pohon silsilah interaktif tanpa batas",
                "Arsip foto & dokumen keluarga",
                "Undang anggota keluarga untuk kolaborasi",
                "Backup data otomatis",
              ],
            },
          ],
          paymentMethods: [
            {
              name: "BCA",
              number: "1234567890",
              holder: "PT Lifestory Indonesia",
            },
            {
              name: "Mandiri",
              number: "0987654321",
              holder: "PT Lifestory Indonesia",
            },
            {
              name: "GoPay / OVO / Dana",
              number: "081234567890",
              holder: "Lifestory",
            },
          ],
          orderMessage: (name: string, email: string, planName: string) =>
            `Halo Admin Lifestory!\n\nSaya ingin berlangganan ${planName}.\n\nNama: ${name}\nEmail: ${email}\n\nMohon konfirmasi pembayaran saya. Terima kasih!`,
          activeTitle: "Langganan Anda Aktif!",
          activeBody:
            "Terima kasih telah menjadi anggota Lifestory. Nikmati semua fitur premium.",
          activeCta: "Buka Family Tree",
          sectionLabel: "Berlangganan",
          heading: "Abadikan Sejarah Keluarga Anda",
          subheading:
            "Bangun pohon silsilah interaktif, simpan kenangan berharga, dan wariskan cerita keluarga untuk generasi mendatang.",
          paymentTitle: "Cara Pembayaran",
          paymentStep1: "Transfer ke salah satu rekening berikut:",
          accountNamePrefix: "a.n.",
          paymentStep2: "Konfirmasi pembayaran via WhatsApp:",
          whatsappCta: "Konfirmasi via WhatsApp",
          paymentStep3: "Akun akan diaktifkan dalam 1x24 jam",
          paymentStep3Body:
            "Admin akan memverifikasi pembayaran dan mengaktifkan akun Anda. Anda akan menerima notifikasi via email setelah langganan aktif.",
          loginPromptPrefix: "Login",
          loginPromptMiddle: "atau",
          loginPromptRegister: "daftar",
          loginPromptSuffix: "terlebih dahulu sebelum berlangganan.",
        }
      : {
          plans: [
            {
              name: "Family Plan",
              price: "Rp 99.000",
              period: "/ month",
              features: [
                "Unlimited interactive family trees",
                "Family photo and document archive",
                "Invite family members to collaborate",
                "Automatic data backup",
              ],
            },
          ],
          paymentMethods: [
            {
              name: "BCA",
              number: "1234567890",
              holder: "PT Lifestory Indonesia",
            },
            {
              name: "Mandiri",
              number: "0987654321",
              holder: "PT Lifestory Indonesia",
            },
            {
              name: "GoPay / OVO / Dana",
              number: "081234567890",
              holder: "Lifestory",
            },
          ],
          orderMessage: (name: string, email: string, planName: string) =>
            `Hello Lifestory Admin!\n\nI want to subscribe to ${planName}.\n\nName: ${name}\nEmail: ${email}\n\nPlease confirm my payment. Thank you!`,
          activeTitle: "Your Subscription Is Active!",
          activeBody:
            "Thank you for becoming a Lifestory member. Enjoy all premium features.",
          activeCta: "Open Family Tree",
          sectionLabel: "Subscription",
          heading: "Preserve Your Family History",
          subheading:
            "Build interactive family trees, store precious memories, and pass down family stories to future generations.",
          paymentTitle: "Payment Instructions",
          paymentStep1: "Transfer to one of the following accounts:",
          accountNamePrefix: "a/n",
          paymentStep2: "Confirm payment via WhatsApp:",
          whatsappCta: "Confirm via WhatsApp",
          paymentStep3: "Account will be activated within 24 hours",
          paymentStep3Body:
            "Admin will verify your payment and activate your account. You will receive an email notification once your subscription is active.",
          loginPromptPrefix: "Login",
          loginPromptMiddle: "or",
          loginPromptRegister: "register",
          loginPromptSuffix: "before subscribing.",
        };

  const selectedPlan = copy.plans[0];
  const orderText = encodeURIComponent(
    copy.orderMessage(
      user?.name || "-",
      user?.email || "-",
      selectedPlan.name
    )
  );

  if (isSubscribed) {
    return (
      <div className="min-h-screen bg-white">
        <section className="mx-auto max-w-2xl space-y-6 px-6 py-16 text-center">
          <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-forest-100">
            <svg
              className="h-8 w-8 text-forest-600"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M5 13l4 4L19 7"
              />
            </svg>
          </div>
          <h1 className="text-3xl font-semibold text-slate-900">
            {copy.activeTitle}
          </h1>
          <p className="text-slate-600">{copy.activeBody}</p>
          <Link href="/app">
            <Button>{copy.activeCta}</Button>
          </Link>
        </section>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      <section className="mx-auto max-w-4xl space-y-10 px-6 py-16">
        <div className="space-y-3 text-center">
          <p className="text-sm font-semibold uppercase tracking-wide text-forest-600">
            {copy.sectionLabel}
          </p>
          <h1 className="text-4xl font-semibold text-slate-900">{copy.heading}</h1>
          <p className="mx-auto max-w-xl text-lg text-slate-600">
            {copy.subheading}
          </p>
        </div>

        {copy.plans.map((plan) => (
          <div
            key={plan.name}
            className="mx-auto max-w-md rounded-2xl border-2 border-forest-200 bg-white p-8 shadow-lg shadow-forest-100/50"
          >
            <div className="space-y-2 text-center">
              <h2 className="text-2xl font-semibold text-slate-900">{plan.name}</h2>
              <div className="flex items-baseline justify-center gap-1">
                <span className="text-4xl font-bold text-forest-700">
                  {plan.price}
                </span>
                <span className="text-slate-500">{plan.period}</span>
              </div>
            </div>
            <ul className="mt-6 space-y-3">
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-start gap-3 text-slate-700">
                  <svg
                    className="mt-1 h-5 w-5 shrink-0 text-forest-500"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                      d="M5 13l4 4L19 7"
                    />
                  </svg>
                  <span>{feature}</span>
                </li>
              ))}
            </ul>
          </div>
        ))}

        <div className="mx-auto max-w-xl space-y-6">
          <h3 className="text-center text-xl font-semibold text-slate-900">
            {copy.paymentTitle}
          </h3>

          <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-6">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-forest-100 text-sm font-semibold text-forest-700">
                1
              </div>
              <p className="text-slate-700">{copy.paymentStep1}</p>
            </div>
            <div className="ml-11 space-y-3">
              {copy.paymentMethods.map((method) => (
                <div key={method.name} className="rounded-lg bg-slate-50 p-3">
                  <p className="font-semibold text-slate-900">{method.name}</p>
                  <p className="text-sm text-slate-600">{method.number}</p>
                  <p className="text-sm text-slate-500">
                    {copy.accountNamePrefix} {method.holder}
                  </p>
                </div>
              ))}
            </div>
          </div>

          <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-6">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-forest-100 text-sm font-semibold text-forest-700">
                2
              </div>
              <p className="text-slate-700">{copy.paymentStep2}</p>
            </div>
            <div className="ml-11">
              <a
                href={`https://wa.me/${WHATSAPP_NUMBER}?text=${orderText}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-green-500 px-6 py-3 font-semibold text-white transition hover:bg-green-600"
              >
                <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                {copy.whatsappCta}
              </a>
            </div>
          </div>

          <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-6">
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-forest-100 text-sm font-semibold text-forest-700">
                3
              </div>
              <p className="text-slate-700">{copy.paymentStep3}</p>
            </div>
            <p className="ml-11 text-sm text-slate-500">{copy.paymentStep3Body}</p>
          </div>
        </div>

        {!user && (
          <div className="mx-auto max-w-md rounded-xl border border-amber-200 bg-amber-50 p-4 text-center">
            <p className="text-amber-800">
              <Link
                href="/auth/login"
                className="font-semibold underline hover:text-amber-900"
              >
                {copy.loginPromptPrefix}
              </Link>{" "}
              {copy.loginPromptMiddle}{" "}
              <Link
                href="/auth/register"
                className="font-semibold underline hover:text-amber-900"
              >
                {copy.loginPromptRegister}
              </Link>{" "}
              {copy.loginPromptSuffix}
            </p>
          </div>
        )}
      </section>
    </div>
  );
}
